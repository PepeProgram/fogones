<?php
    ...
    if ($datos->rowCount()<=0) {
        ...
        $receta_actualizar = new recetaModel($datos['id_receta'], $datos['nombre_receta'], $datos['descripcion_receta'], $datos['id_usuario'], $datos['id_grupo'], $datos['n_personas'], $datos['tiempo_receta'], $datos['id_autor'], $datos['id_region'], $datos['id_pais'], $datos['id_zona'], $datos['dificultad'], $datos['elaboracion'], $datos['emplatado'], $datos['foto_receta'], $datos['visualizaciones'], $datos['creado_receta'], $datos['actualizado_receta'], $datos['activo']);
    }

    /* Comprueba si el usuario es administrador, revisor o la receta es suya */
    if (!isset($_SESSION['administrador'])) {

        /* Comprueba si el usuario es revisor */
        if (!isset($_SESSION['revisor'])) {

            /* Comprueba si la receta pertenece al usuario */
            if ($_SESSION['id'] != $receta_actualizar->getId_usuario()) {
                echo "
                <script>
                textoAlerta = {
                    tipo: 'redirigir',
...
<script type="text/javascript">
    ...
    window.addEventListener('load', async function(){
        ...
        /* Rellena el select de zonas geográficas seleccionando la de la receta si la tiene */
        rellenarSelect('', 'zonaEnviarReceta', 'zonas', '', '<?php echo $receta_actualizar->getId_zona() ?>');

        /* Si la receta tiene zona, rellena el select de países seleccionando el de la receta si lo tiene */
        <?php
            if ($receta_actualizar->getId_zona() != 0) {
                echo "rellenarSelect('".$receta_actualizar->getId_zona()."', 'paisEnviarReceta', 'paises', 'id_zona', ".$receta_actualizar->getId_pais().");";
            } 
        
        ?>

        /* Si la receta tiene país, rellena el select de regiones seleccionando la de la receta si la tiene */
        <?php
            if ($receta_actualizar->getId_pais() != 0) {
                echo "rellenarSelect('".$receta_actualizar->getId_pais()."', 'regionEnviarReceta', 'regiones', 'id_pais', ".$receta_actualizar->getId_region().");";
            } 
        
        ?>

        /* Rellena el select de Estilos de cocina */
        rellenarSelect('', 'estiloCocinaEnviarReceta', 'estilos_cocina', '', <?php echo json_encode($receta_actualizar->getEstilos()) ?>);

        /* Rellena el select de Tipos de plato */
        rellenarSelect('', 'tipoPlatoEnviarReceta', 'tipos_plato', '', <?php echo json_encode($receta_actualizar->getTipos_plato()) ?>);
        
        /* Rellena el select de Métodos de cocción */
        rellenarSelect('', 'metodoEnviarReceta', 'tecnicas', '', <?php echo json_encode($receta_actualizar->getMetodos()) ?>);
...
<form action="<?php echo APP_URL; ?>app/ajax/recetaAjax.php" id="formEnviarReceta" method="POST" class="FormularioAjax" enctype="multipart/form-data" name="Guardar Receta?">
    ...
    <input type="hidden" name="modulo_receta" value="actualizar">
    <input type="hidden" name="id_receta" value="<?php echo $receta_actualizar->getId(); ?>">
    ...
    <section name="resumen" id="cabeceraEnviarReceta" class="cabeceraEnviarReceta col-100 horizontal total">
        ...
        <!-- Foto de la receta -->
         <?php 
            /* Comprueba si la receta tiene foto o no */
            if ($receta_actualizar->getFoto() != "") {
                $foto_receta = $receta_actualizar->getFoto();
                $alt_foto = "Foto de ";
            } else {
        ...
        <div id="containerDatosPrincipalesEnviarReceta" class="containerDatosPrincipalesEnviarReceta col-75 total derecha vertical">
            ...
            <div class="nombrePrincipalesInput medio izquierda derecha top horizontal">
                <div class="vertical col-40 medio izquierda top">
                    ...
                    <input type="text" name="nombreReceta" id="nombreReceta" class="input  nombreReceta" maxlength="255" pattern="[\(\)a-zA-Z0-9áéíóúÁÉÍÓÚñÑ.,:;%\/\-_ ]{3,255}" title="Introduzca el nombre del Tipo de platos. Sólo puede contener letras, números, (, ), , ,, ;, :, %, .,-,_ y espacios, entre 3 y 255 caracteres" value="<?php echo $receta_actualizar->getNombre(); ?>">
                </div>
                <div class="vertical col-20 medio top">
                    <label for="numeroPersonasEnviarReceta" class="labelForm">Pax.*</label>
                    <input type="number" name="numeroPersonasEnviarReceta" id="numeroPersonasEnviarReceta" class="input numeroPersonasEnviarReceta" min="1" value="<?php echo $receta_actualizar->getPersonas(); ?>">
                </div>
                <div class="vertical col-20 medio top">
                ...
                <div class="vertical col-20 medio derecha top">
                    ...
                    <select name="dificultadEnviarReceta" id="dificultadEnviarReceta" class="input dificultadEnviarReceta">
                        <option value="0" disabled <?= ($receta_actualizar->getDificultad() == 0) ? 'selected' : '' ?>>Seleccione</option>
                        <option value="1" class="selectDificultadEnviarReceta" <?= ($receta_actualizar->getDificultad() == 1) ? 'selected' : '' ?>>
                            &starf;&star;&star;&star;&star;
                        </option>
                        <option value="2" class="selectDificultadEnviarReceta" <?= ($receta_actualizar->getDificultad() == 2) ? 'selected' : '' ?>>
                            &starf;&starf;&star;&star;&star;
                        </option>
                        <option value="3" class="selectDificultadEnviarReceta" <?= ($receta_actualizar->getDificultad() == 3) ? 'selected' : '' ?>>
                            &starf;&starf;&starf;&star;&star;
                        </option>
                        <option value="4" class="selectDificultadEnviarReceta" <?= ($receta_actualizar->getDificultad() == 4) ? 'selected' : '' ?>>
                            &starf;&starf;&starf;&starf;&star;
                        </option>
                        <option value="5" class="selectDificultadEnviarReceta" <?= ($receta_actualizar->getDificultad() == 5) ? 'selected' : '' ?>>
            ...
            <div id="descripcionEnviarReceta" class="descripcionEnviarReceta col-100 vertical">
                ...
                <textarea name="descripcionCorta" rows="4" id="descripcionCorta" class="descripcionCorta inputText" pattern="[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ.,:\/\-_ ]{3,255}" maxlength="255" title="Introduzca una descripción corta. Sólo puede contener letras, números, ., ,, :, -,/,_ y espacios. Mínimo 3 caracteres y máximo 255"><?php echo $receta_actualizar->getDescripcion(); ?></textarea>
            </div>
        </div>
    </section>
